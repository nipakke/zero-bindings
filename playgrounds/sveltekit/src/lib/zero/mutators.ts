...
/** Client-only transactions never wrap a database connection. */
...
/**
 * Mutator registry handed to `createDefinitions`. Mutators must be idempotent:
 * Zero re-runs them whenever server changes are folded back in. Every write
 * is keyed by primary key, so re-running is safe.
 */
